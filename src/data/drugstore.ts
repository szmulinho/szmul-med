...
export async function getAllDrugs(): Promise<Drug[]> {
    const response = await axios.get('https://3.75.158.163:8081/drugs');
...
export async function getDrugByName(id: string): Promise<void> {
    try {
        await axios.get(`https://3.75.158.163:8081/drugs/${name}`);
    } catch (error) {
...
export async function postDrug(postData: Drug): Promise<Drug[]> {
    const response = await axios.post('https://3.75.158.163:8081/drug', postData);
    return response.data;
}

export async function deleteDrug(id: string): Promise<void> {
    try {
        await axios.delete(`https://3.75.158.163:8081/drugs/${id}`);
    } catch (error) {
...
export async function updateDrug(id: string, updatedDrug: Partial<Drug>): Promise<Drug> {
    const response = await axios.patch(`https://3.75.158.163:8081/drugs/${id}`, updatedDrug);
    return response.data;
}

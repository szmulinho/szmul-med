...
export function ApapProfile(): JSX.Element {
    const { name } = useParams<RouteParams>();
    const [drug, setDrug] = useState<Drug | null>(null);
    const [quantity, setQuantity] = useState<number>(1);
    const { increaseCartQuantity } = useShoppingCart(); // Access the increaseCartQuantity function from the context

    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`https://szmul-med-drugstore.onrender.com/drugs/1`);
    ...
    const handleAddToCart = () => {
        if (drug) {
            increaseCartQuantity(drug.drug_id); // Call the context function to add the drug to the cart
        }
    };
